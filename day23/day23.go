...
func (p *packet) String() string {
	return fmt.Sprintf("{%d:%d,%d}", p.address, p.x, p.y)
}
...
func (n *network) init(size int) {
	n.packets = make([][]packet, size)
	for i := range n.packets {
		n.packets[i] = make([]packet, 0)
	}
	n.idleMask = 0
	n.idleCount = 0
}
...
func (n *network) isIdle() bool {
	allIdle := 1<<50 - 1
	//if the idle mask is not all 1ses we aren't idle
	if allIdle^n.idleMask != 0 {
		n.idleCount = 0
		return false
	}

	//if there's an item on a queue we aren't idle
...
func (n *network) send(p packet, fromAdr int) {
	n.mux.Lock()
	n.idleMask = 0
	if p.address == 255 {
		n.natPacket = p
		...
		n.packets[p.address] = append(n.packets[p.address], p)
	}
	defer n.mux.Unlock()
}

func (n *network) receive(adr int) (packet, bool) {
	n.mux.Lock()
	exists := false
	p := packet{}
	if len(n.packets[adr]) > 0 {
		//pop that sucker
		p = n.packets[adr][0]
		n.packets[adr] = n.packets[adr][1:]
		exists = true
		n.idleMask = 0
	} else if n.isIdle() && adr == 0 {
		fmt.Println("Repriming from NAT")
		exists = true
		p = n.natPacket
		if n.lastSent.y == n.natPacket.y {
			fmt.Printf("OH OH! I'm delivering %d for the second time in a row!\n", n.lastSent.y)
			os.Exit(0)
		}
		n.lastSent = n.natPacket
		n.idleMask = 0
		defer n.mux.Unlock()
		return p, exists
	} else {
		n.idleMask = n.idleMask | (1 << adr)
	}
...
//DoTheThing that day 23 is supposed to do
func DoTheThing() {
