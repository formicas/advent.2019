...
func DoTheThing() {
	...
	readUntilPrompted(output)
	writeInput(main, input)
	readUntilPrompted(output)
	writeInput(a, input)
	readUntilPrompted(output)
	writeInput(b, input)
	...
	writeInput(c, input)
	readUntilPrompted(output)
	writeInput(video, input)
	readUntilEnd(output)
	//fmt.Printf("Dust: %d", <-output)
}

func readUntilPrompted(output chan int) {
	promptHit := false
	for {
		char := <-output
		r := rune(char)
		fmt.Print(string(r))
		if char == 10 && promptHit {
			break
		}
		if r == ':' || r == '?' {
			promptHit = true
...
func readUntilEnd(output chan int) {
	for {
		char := <-output
		if char > 94 {
			fmt.Printf("Dust: %d", char)
			break
		}
		fmt.Print(string(rune(char)))

		if rune(char) == 'X' {
			fmt.Println("The robot died")
			break
		}
...
func writeInput(message string, input chan int) {
